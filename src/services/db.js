...
const db_sensor = new PouchDB(db_local_prefix + 'sensor');
...
const db_remote_sensor = new PouchDB(conn_prefix + 'sensor', {auth: auth});
...
db_sensor.sync(db_remote_sensor, sync_options)
.on('change', (info) => {
  console.log('CHANGE');
  console.log(info);
  sync_monitor.set_active();
