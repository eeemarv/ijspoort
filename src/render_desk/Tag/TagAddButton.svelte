<script>
  ...
  $: if (!$selected_person_id || !type_id){
    disabled = true;
  } else if (!$tag_type_map.has(type_id)){
    disabled = true;
  } else if ($tag_type_map.get(type_id).max_per_person < 1){
    disabled = true;
  } else if ($person_tag_map.has($selected_person_id)){
    if (!$person_tag_map.get($selected_person_id).has(type_id)){
      disabled = false;
    } else if ($person_tag_map.get($selected_person_id).get(type_id).size
...
<Button
  ...
  color=success
  title="Tag toevoegen"
  on:click={() => {tag_add_bulk([type_id], $selected_person_id);}}
  {disabled}
>
  <Icon icon={plusIcon} />
</Button>
